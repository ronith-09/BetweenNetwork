...
func (pc *ParticipantChaincode) verifyParticipantBankAccess(
	ctx contractapi.TransactionContextInterface,
	bankID string,
	functionName string,
	...
	callerMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return nil, "", fmt.Errorf("[AUTHORIZATION FAILED] %s: failed to get caller MSP ID: %v", functionName, err)
	}

	clientID, err := ctx.GetClientIdentity().GetID()
	if err != nil {
		return nil, "", fmt.Errorf("[AUTHORIZATION FAILED] %s: failed to get caller certificate ID: %v", functionName, err)
	}
	...
	if participant.MspID != callerMSPID {
		return nil, "", fmt.Errorf(
			"[UNAUTHORIZED ACCESS DENIED] %s: caller MSP '%s' cannot act for bank '%s' owned by MSP '%s'",
			functionName,
			callerMSPID,
			bankID,
			participant.MspID,
	...
	return participant, clientID, nil
}

func (pc *ParticipantChaincode) verifySettlementPartyAccess(
	ctx contractapi.TransactionContextInterface,
	...
	fromParticipant, err := pc.GetParticipant(ctx, settlement.FromBank)
	if err != nil {
		return "", err
	}

	toParticipant, err := pc.GetParticipant(ctx, settlement.ToBank)
	if err != nil {
		return "", err
	}

	if callerMSPID != fromParticipant.MspID && callerMSPID != toParticipant.MspID {
		return "", fmt.Errorf(
			"[UNAUTHORIZED ACCESS DENIED] %s: caller MSP '%s' is not a party to settlement '%s'",
			functionName,
			callerMSPID,
...
func (pc *ParticipantChaincode) verifySettlementReadAccess(
	ctx contractapi.TransactionContextInterface,
	settlement *Settlement,
	functionName string,
) error {
	...
	if callerMSPID == "BetweenMSP" {
		return nil
	}

	_, err = pc.verifySettlementPartyAccess(ctx, settlement, functionName)
	return err
}

func (pc *ParticipantChaincode) validateSettlementInput(
	ctx contractapi.TransactionContextInterface,
	fromBank string,
	toBank string,
	currency string,
...
func (pc *ParticipantChaincode) hasSufficientBalanceForBank(
	ctx contractapi.TransactionContextInterface,
	bankID string,
	currency string,
	amount string,
...
func (pc *ParticipantChaincode) GetWallet(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "GetWallet"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) CreateMintRequest(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "CreateMintRequest"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) GetOwnMintRequests(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "GetOwnMintRequests"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) GetOwnMintRequestById(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "GetOwnMintRequestById"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) ValidateSettlement(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, fromBank, "ValidateSettlement"); err != nil {
		return false, err
	}

	return pc.validateSettlementInput(ctx, fromBank, toBank, currency, amount)
}
...
func (pc *ParticipantChaincode) HasSufficientBalance(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "HasSufficientBalance"); err != nil {
		return false, err
	}

	return pc.hasSufficientBalanceForBank(ctx, bankID, currency, amount)
}
...
func (pc *ParticipantChaincode) CheckDuplicateSettlement(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, fromBank, "CheckDuplicateSettlement"); err != nil {
		return false, err
	}
...
func (pc *ParticipantChaincode) CreateSettlementRequest(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, fromBank, "CreateSettlementRequest"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) ExecuteSettlement(
	...
	if _, err := pc.validateSettlementInput(ctx, settlement.FromBank, settlement.ToBank, settlement.Currency, strconv.FormatInt(settlement.Amount, 10)); err != nil {
		return nil, err
	}

	hasBalance, err := pc.hasSufficientBalanceForBank(ctx, settlement.FromBank, settlement.Currency, strconv.FormatInt(settlement.Amount, 10))
	if err != nil {
		return nil, err
	}
	if !hasBalance {
...
// GetSettlementById returns a settlement by ID for BetweenNetwork admin or a participant party.
func (pc *ParticipantChaincode) GetSettlementById(
	...
	if err := pc.verifySettlementReadAccess(ctx, settlement, "GetSettlementById"); err != nil {
		return nil, err
	}
...
// GetSettlementStatus returns the settlement status for BetweenNetwork admin or a participant party.
func (pc *ParticipantChaincode) GetSettlementStatus(
	...
	if err := pc.verifySettlementReadAccess(ctx, settlement, "GetSettlementStatus"); err != nil {
		return "", err
	}
...
func (pc *ParticipantChaincode) GetOwnSettlementHistory(
	...
	if _, _, err := pc.verifyParticipantBankAccess(ctx, bankID, "GetOwnSettlementHistory"); err != nil {
		return nil, err
	}
...
func (pc *ParticipantChaincode) InvestigateSettlement(
	...
	if _, err := pc.verifySettlementPartyAccess(ctx, settlement, "InvestigateSettlement"); err != nil {
		return nil, err
	}
